...
describe 'TaskRouter ' do
  ...
  describe Textgrid::JWT::TaskRouterCapability do
    describe 'token initialization' do
      before :each do
        @taskRouterCapability = Textgrid::JWT::TaskRouterCapability.new 'AC123', 'secret', 'WS456', 'CI123'
      end
      ...
      it 'add policy' do
        @policy = Textgrid::JWT::TaskRouterCapability::Policy.new(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.workspace('WK123'), 'GET', true)
  ...
  describe Textgrid::JWT::TaskRouterCapability::Policy do
    describe 'policy' do
      it 'initialize' do
        @policy = Textgrid::JWT::TaskRouterCapability::Policy.new('test-url',
                                                                'GET',
                                                                true,
    ...
    describe Textgrid::JWT::TaskRouterCapability::TaskRouterUtils do
      describe 'urls' do
        it 'workspaces' do
          expected_url = 'https://taskrouter.twilio.com/v1/Workspaces'
          expect(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.workspaces).to eq(expected_url)
        end

        it 'workspace' do
          expected_url = 'https://taskrouter.twilio.com/v1/Workspaces/WK123'
          expect(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.workspace('WK123')).to eq(expected_url)
        end

        it 'all_workspaces' do
          expected_url = 'https://taskrouter.twilio.com/v1/Workspaces/**'
          expect(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.all_workspaces).to eq(expected_url)
        end

        it 'tasks' do
          expected_url = 'https://taskrouter.twilio.com/v1/Workspaces/WK123/Tasks'
          expect(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.tasks('WK123')).to eq(expected_url)
        end

        it 'task' do
          expected_url = 'https://taskrouter.twilio.com/v1/Workspaces/WK123/Tasks/TK123'
          expect(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.task('WK123', 'TK123')).to eq(expected_url)
        end

        it 'all_tasks' do
          expected_url = 'https://taskrouter.twilio.com/v1/Workspaces/WK123/Tasks/**'
          expect(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.all_tasks('WK123')).to eq(expected_url)
        end

        it 'task_queues' do
          expected_url = 'https://taskrouter.twilio.com/v1/Workspaces/WK123/TaskQueues'
          expect(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.task_queues('WK123')).to eq(expected_url)
        end

        it 'task_queue' do
          expected_url = 'https://taskrouter.twilio.com/v1/Workspaces/WK123/TaskQueues/WQ123'
          expect(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.task_queue('WK123', 'WQ123')).to eq(expected_url)
        end

        it 'all_task_queues' do
          expected_url = 'https://taskrouter.twilio.com/v1/Workspaces/WK123/TaskQueues/**'
          expect(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.all_task_queues('WK123')).to eq(expected_url)
        end

        it 'activities' do
          expected_url = 'https://taskrouter.twilio.com/v1/Workspaces/AT123/Activities'
          expect(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.activities('AT123')).to eq(expected_url)
        end

        it 'activity' do
          expected_url = 'https://taskrouter.twilio.com/v1/Workspaces/WK123/Activities/AT123'
          expect(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.activity('WK123', 'AT123')).to eq(expected_url)
        end

        it 'all_activities' do
          expected_url = 'https://taskrouter.twilio.com/v1/Workspaces/WK123/Activities/**'
          expect(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.all_activities('WK123')).to eq(expected_url)
        end

        it 'workers' do
          expected_url = 'https://taskrouter.twilio.com/v1/Workspaces/WK123/Workers'
          expect(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.workers('WK123')).to eq(expected_url)
        end

        it 'worker' do
          expected_url = 'https://taskrouter.twilio.com/v1/Workspaces/WK123/Workers/WT123'
          expect(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.worker('WK123', 'WT123')).to eq(expected_url)
        end

        it 'all_workers' do
          expected_url = 'https://taskrouter.twilio.com/v1/Workspaces/WK123/Workers/**'
          expect(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.all_workers('WK123')).to eq(expected_url)
        end

        it 'reservations' do
          expected_url = 'https://taskrouter.twilio.com/v1/Workspaces/WK123/Workers/WT123/Reservations'
          expect(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.reservations('WK123', 'WT123')).to eq(expected_url)
        end

        it 'reservation' do
          expected_url = 'https://taskrouter.twilio.com/v1/Workspaces/WK123/Workers/WT123/Reservations/RS123'
          expect(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.reservation('WK123', 'WT123', 'RS123')).to eq(expected_url)
        end

        it 'all_reservations' do
          expected_url = 'https://taskrouter.twilio.com/v1/Workspaces/WK123/Workers/WT123/Reservations/**'
          expect(Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.all_reservations('WK123', 'WT123')).to eq(expected_url)
        end
      end

      describe 'default policies' do
        it 'default web_socket_policies' do
          @policies = Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.web_socket_policies('AC123', 'CI123')
        ...
        it 'default worker policies' do
          @policies = Textgrid::JWT::TaskRouterCapability::TaskRouterUtils.worker_policies('WK123', 'WT123')
